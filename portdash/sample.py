...
def _create_single_buy_transactions(
    dates: pd.DatetimeIndex, contribution: float, symbol: str, account: str
) -> pd.DataFrame:
...
def _create_sample_acct_trans(
    dates: pd.DatetimeIndex, contribution: float, invest_trans: pd.DataFrame
) -> pd.DataFrame:
    ...
    acct_trans = (
        pd.concat(
            [
                ...
                pd.DataFrame(
                    {
                        "Date": invest_trans["Date"],
                        "Payee": invest_trans["Symbol"],
                        "Category": invest_trans.apply(_acct_cat_str, axis="columns"),
                        "S": "R",
                        "Withdrawal": invest_trans["Total"],
                        "Deposit": 0.0,
                        "Total": 0.0,
                        "Comment": "",
                    },
...
def _acct_cat_str(row: pd.Series) -> str:
    ...
    if row["Action"].startswith("Reinvest"):
        return (
            f"{row['Action']} {row['Dividend']} {row['Quantity']} "
            f"{row['Symbol']} @ {row['Price']}"
        )
    elif row["Action"] in ["Buy", "Sell"]:
        return f"{row['Action']} {row['Quantity']} " f"{row['Symbol']} @ {row['Price']}"
    else:
...
def create_transactions(
    ...
    for symbol, frac in security_mix.items():
        ...
        buy = _create_single_buy_transactions(
            date_range, this_contribution, symbol, account
        )
        transact_list.append(_include_reinvest_dividend(buy, date_range))
    invest_trans = (
        pd.concat(transact_list)
        .sort_values(by="Date", ascending=True)
        .reset_index(drop=True)
    )
    acct_trans = _create_sample_acct_trans(
        date_range, monthly_contribution, invest_trans
    )
...
def write_sample_data(
    ...
    invest_trans, acct_trans = create_transactions(
        monthly_contribution, security_mix, start_date, stop_date, account
    )

    os.makedirs(config.conf("data_dir"), exist_ok=True)
    invest_trans.to_csv(
        config.conf("investment_transactions"), index=False, quoting=csv.QUOTE_ALL
    )
    acct_trans.to_csv(
        config.conf("account_transactions")[account], index=False, quoting=csv.QUOTE_ALL
    )

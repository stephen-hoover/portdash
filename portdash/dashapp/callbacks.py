...
def relative_gain(account, n_days, annualize=False, ignore_zeros=False):
    ...
    avg_pct_change = (1 + (change / shift_tot)).rolling(f"{n_days}d").apply(np.prod)
    if annualize:
...
def get_stop_time(date_range: str, max_range: datetime) -> Union[datetime, None]:
    if date_range == "max":
...
def _total_ror(account_names, start_date, stop_date, sim_symbol=None):
    ...
    stop_date = max([min([last_date, stop_date]), start_date + timedelta(days=1)])
    n_days = calc_n_days(start_date, stop_date)
    if sim_symbol:
        acct = create_simulated_portfolio(acct.loc[start_date:], sim_symbol)
    y = relative_gain(acct.loc[:stop_date], n_days, annualize=True, ignore_zeros=True)
    return y.iloc[-1]


def calculate_traces(account_names, start_time=None, stop_time=None, date_range=None):
    if not account_names:
        raise NoUpdate()
...
def register_callbacks(app):
    @app.callback(
        dash.dependencies.Output("sim-selector-container", "hidden"),
    ...
    @app.callback(
        dash.dependencies.Output("portfolio-value", "relayoutData"),
    ...
    @app.callback(
        dash.dependencies.Output("date-selector", "start_date"),
        [
            dash.dependencies.Input("date-range", "value"),
    ...
    @app.callback(
        dash.dependencies.Output("date-selector", "end_date"),
        [
            dash.dependencies.Input("date-range", "value"),
            dash.dependencies.Input("portfolio-value", "relayoutData"),
        ],
    ...
    @app.callback(
        dash.dependencies.Output("value", "children"),
        [
    ...
    @app.callback(
        dash.dependencies.Output("rate-of-return", "children"),
        [
            dash.dependencies.Input("portfolio-selector", "value"),
            dash.dependencies.Input("comparison-selector", "value"),
            dash.dependencies.Input("date-selector", "start_date"),
    ...
    @app.callback(
        dash.dependencies.Output("line-selector", "multi"),
        [dash.dependencies.Input("comparison-selector", "value")],
    )
    ...
    @app.callback(
        dash.dependencies.Output("portfolio-value", "figure"),
        [
            dash.dependencies.Input("portfolio-selector", "value"),
    ...
    def make_graph(
        account_names, lines, comp_type, min_date, max_date, n_clicks, sim_symbol
    ):
        lines = np.atleast_1d(lines)
        if comp_type == "compare":
            return _make_comparison_graph(account_names, lines, min_date, max_date)
        elif comp_type == "sim":
            return _make_counterfactual_graph(
                account_names, sim_symbol, lines, min_date, max_date
            )
        else:

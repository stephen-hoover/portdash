...
class Quote(db.Model):
    ...
    symbol = db.Column(db.Integer, db.ForeignKey("securities.symbol"), nullable=False)
    security = db.relationship("Security", backref=db.backref("quotes", lazy="select"))
    ...
    def insert(cls, df: pd.DataFrame, symbol: str):
        ...
        if "price" not in df or "volume" not in df:
            raise TypeError(
                f'The input table must have "price" and ' f'"volume" columns.'
            )
        sql = (
        ...
        existing = pd.read_sql(
            sql=sql, con=db.engine, parse_dates=["date"], index_col="date"
        )
    ...
    def most_recent(cls, symbol: str = None) -> "Quote":
        if symbol:
            qu = (
                cls.query.filter(cls.symbol == symbol).order_by(cls.date.desc()).first()
            )
        else:
    ...
    def get_price_at_date(cls, symbol: str, date: datetime) -> float:
        row = cls.query.filter(db.and_(cls.symbol == symbol, cls.date == date)).first()
        if not row:
    ...
    def get_price(
        ...
        if not len(df):
            ...
            log.debug(
                f"Unable to find quotes for {symbol} in the "
                f"database. Filling with {default}."
            )
            df = pd.DataFrame(
                {"price": [default]}, index=pd.DatetimeIndex(["2015-01-01"])
            )
    ...
    def __repr__(self):
        return (
            f"Quote(date={self.date}, price={self.price}, "

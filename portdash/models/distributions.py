...
class Distribution(db.Model):
    ...
    security = db.relationship(
        "Security", backref=db.backref("distributions", lazy="select")
    )
    ...
    def insert(cls, df: pd.DataFrame, symbol: str):
        ...
        if "amount" not in df:
            raise TypeError(
                f'The input table must have an "amount" or '
    ...
    @classmethod
    def get_amount(
        cls, symbol: str, index: Union[pd.DatetimeIndex, Sequence[datetime]] = None
    ) -> pd.Series:
        df = pd.read_sql(
            sql=(
                db.session.query(cls)
        ...
        if index is not None:
            amount = df.loc[
                (df.index >= index.min()) & (df.index <= index.max()), "amount"
            ]
        else:
            amount = df["amount"]
        return amount
    ...
    def __repr__(self):
        return (
            f"Distribution(date={self.date}, amount={self.amount}, "
